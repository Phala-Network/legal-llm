...
class CaseRetriever:
    ...
    def _init_bm25(self):
        if os.path.exists(self.bm25_path):
            ...
            tokenized_corpus = [doc.split() for doc in texts] # Simple whitespace tokenizer
    ...
    def search_vector(self, query, k=25):
        embedding = self.client.embeddings.create(input=[query], model=self.embedding_model_name).data[0].embedding
    ...
    def retrieve(self, query, k=5):
        ...
        if bm25_only_ids:
            try:
                metas = self.collection.get(ids=bm25_only_ids, include=['metadatas', 'documents'])
                for i, mid in enumerate(metas['ids']):
                    if mid in combined:
                        combined[mid]['metadata'] = metas['metadatas'][i]
                        combined[mid]['text'] = metas['documents'][i] # Ensure text is there
            except:
                pass

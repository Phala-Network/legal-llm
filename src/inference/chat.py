...
class LawAssistant:
    ...
    def chat(self, user_query):
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a helpful legal assistant. To search for cases, output a search query wrapped in tags, strictly like this: <search>your query terms</search>. Do not use function call syntax like search(...).",
            },
        ...
        for turn in range(max_turns):
            ...
            new_response = self.tokenizer.decode(
            ...
            if not search_match:
                ...
                if fallback_match:
                    ...
                    print(
                        "DEBUG: Detected non-standard search(...) format, handling gracefully."
                    )
            ...
            if search_match:
                ...
                for i, doc in enumerate(retrieved_docs):
                    context_str += (
                        f"[Result {i+1}] {doc['metadata'].get('name', 'Case')}\n"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--query", type=str, required=True, help="Legal question to ask"
    )
